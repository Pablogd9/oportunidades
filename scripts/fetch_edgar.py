...
def validate_cache():
    ...
    for etf in UNIVERSE:
        ...
        if not cached: issues.append(f"  ⚠️  {symbol}: no encontrado"); continue
        if cached.get("count",0)<20: issues.append(f"  ⚠️  {symbol}: solo {cached.get('count',0)} dias"); continue
        print(f"  ✓ {symbol:10s} {cached['count']:5d} dias | {cached.get('first')} → {cached.get('last')}")
    if issues: print("\nProblemas:"); [print(i) for i in issues]
    else: print("\n✓ Cache validado correctamente")
    return len(issues)==0
...
def print_summary():
    ...
    for group_name,group in [("BACKTEST",[e for e in UNIVERSE if "backtest" in e["use"]]),
        ...
        for etf in group:
            ...
            else: print(f"  {etf['symbol']:10s} ⚠️ sin datos")
    ...
    for etf in [e for e in UNIVERSE if "backtest" in e["use"]]:
        ...
        if cached and cached.get("count",0)>0:
            try:
                ...
                if years>5: s=int((years-5)*12); total+=s; print(f"  {etf['symbol']:10s} {years:.1f}A → ~{s} señales")
            except: pass
    print(f"\n  TOTAL señales estimadas: ~{total}")
    if total>200:   print("  ✓✓ EXCELENTE — p-value < 0.05 alcanzable")
    elif total>120: print("  ✓  BUENO — potencia estadistica suficiente")
    elif total>60:  print("  ~  ACEPTABLE")
    else:           print("  ⚠️  INSUFICIENTE")
    print("="*65)
...
def main():
    ...
    print(f"{'Descarga completa' if force_full else 'Actualizando cache'} — {len(UNIVERSE)} simbolos\nCache: {CACHE}\n")
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                elif s and s.startswith("error"): print(f"  ✗ {sym:12s} {s}"); errors.append(sym)
    ...
    up=sum(1 for r in results if r["status"]=="updated")
    sk=sum(1 for r in results if r["status"]=="skip")
    print(f"\n{'='*65}\nCompletado en {elapsed}s | Descargados:{dl} Actualizados:{up} Sin cambios:{sk} Errores:{len(errors)}")
