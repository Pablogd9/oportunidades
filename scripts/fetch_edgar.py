...
La SEC publica trimestralmente los holdings exactos de cada ETF
registrado en EEUU en el formulario N-PORT. Completamente gratuito.

Prioridad: cache local → EDGAR → Yahoo Finance → fallback hardcodeado
"""
    ...
    try:
        ...
        if "InvstOrSecs" in investments:
            for item in investments["InvstOrSecs"].get("units",{}).get("USD",[]):
                ...
                val    = item.get("val",0)
                if ticker and val>0:
                    holdings_raw[ticker] = holdings_raw.get(ticker,0) + val
...
def fetch_holdings_yahoo(symbol):
    try:
        url = f"https://query1.finance.yahoo.com/v10/finance/quoteSummary/{symbol}?modules=topHoldings"
        d   = _get(url)
...
def get_holdings(symbol):
    """Prioridad: cache → EDGAR → Yahoo → fallback hardcodeado."""
