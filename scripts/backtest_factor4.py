...
# -*- coding: utf-8 -*-
"""
backtest_factor4.py — Validacion temporal por mitades (out-of-sample split).

Mitad 1: 2017-2021 (boom covid + caida)
Mitad 2: 2022-2026 (subida tipos + IA/defensa)
"""
...
OUT         = os.path.join(ROOT, "data", "backtest_factor4.json")
...
def analizar_periodo(nombre, snaps_periodo, ids, etf_data, iwda_p, iwda_d, months_periodo):
    ...
    tops=Counter(s["top1_sym"] for s in snaps_periodo)
    print(f"\n    Sectores mas elegidos en {nombre}:")
    for sym,n in tops.most_common(5):
        ss=[s for s in snaps_periodo if s["top1_sym"]==sym]
    ...
    return {"n":len(snaps_periodo),"alpha_medio":am,"bate_pct":bate,
            ...
            "top_sectores":{sym:n for sym,n in tops.most_common(5)}}

def main():
    print("="*70)
    print("BACKTEST FACTOR 4 — Validacion temporal por mitades")
    print(f"Fecha: {TODAY} | ETFs: {len(BACKTEST_UNIVERSE)} | Corte: {SPLIT_DATE}")
    print("="*70)
    ...
    print(f"Corte de mitades: {SPLIT_DATE}")
    ...
    for y,m in months:
        ...
        snaps.append({"date":ed.isoformat(),"year":y,"month":m,"top1_sym":t1["sym"],
                     "top1_score":t1["score"],"top1_ret":t1["ret"],"ri":ri,
                     "alpha":round(t1["ret"]-ri,2)})

    if not snaps: print("ERROR sin señales"); return

    mitad1_snaps=[s for s in snaps if datetime.date.fromisoformat(s["date"]) < SPLIT_DATE]
    mitad2_snaps=[s for s in snaps if datetime.date.fromisoformat(s["date"]) >= SPLIT_DATE]
    mitad1_months=[m for m in months if datetime.date(m[0],m[1],1) < SPLIT_DATE]
    mitad2_months=[m for m in months if datetime.date(m[0],m[1],1) >= SPLIT_DATE]

    print(f"\nMitad 1 (2017-2021): {len(mitad1_snaps)} señales")
    print(f"Mitad 2 (2022-2026): {len(mitad2_snaps)} señales")

    ids=list(etf_data.keys())

    print("\n"+"="*70)
    print("ANALISIS POR MITADES")
    print("="*70)

    r1=analizar_periodo("MITAD 1: 2017-2021 (boom covid + caida)", mitad1_snaps, ids, etf_data, iwda_p, iwda_d, mitad1_months)
    r2=analizar_periodo("MITAD 2: 2022-2026 (subida tipos + IA/defensa)", mitad2_snaps, ids, etf_data, iwda_p, iwda_d, mitad2_months)

    print("\n"+"="*70)
    print("COMPARACION DIRECTA")
    print("="*70)
    if r1 and r2:
        print(f"\n  {'Metrica':25s} {'Mitad 1':>12s} {'Mitad 2':>12s}")
        print(f"  {'-'*50}")
        print(f"  {'Alpha medio':25s} {r1['alpha_medio']:>+11.2f}% {r2['alpha_medio']:>+11.2f}%")
        print(f"  {'Bate IWDA %':25s} {r1['bate_pct']:>11.1f}% {r2['bate_pct']:>11.1f}%")
        print(f"  {'P-value Newey-West':25s} {str(r1['pvalue_nw']):>12s} {str(r2['pvalue_nw']):>12s}")
        print(f"  {'P-value Bootstrap':25s} {str(r1['pvalue_bootstrap']):>12s} {str(r2['pvalue_bootstrap']):>12s}")

        ambas_positivas = r1['alpha_medio']>0 and r2['alpha_medio']>0
        ambas_significativas = (r1['pvalue_nw'] is not None and r1['pvalue_nw']<0.20 and
                                 r2['pvalue_nw'] is not None and r2['pvalue_nw']<0.20)

        print(f"\n  CONCLUSION:")
        if ambas_positivas and ambas_significativas:
            print(f"  ✓✓ El efecto es CONSISTENTE en ambas mitades. Evidencia de alpha real.")
        elif ambas_positivas:
            print(f"  ✓  Alpha positivo en ambas mitades, pero significancia debil en al menos una.")
        else:
            print(f"  ✗  El alpha NO es consistente entre mitades.")
            print(f"     Sugiere que depende de una racha concentrada, no de un efecto estructural.")

    print("\n"+"="*70)
    print("DIAGNOSTICO: COPX y XBI por mitad")
    print("="*70)

    for sym_check in ["COPX","XBI"]:
        obs_m1=[o for o in all_obs if o["sym"]==sym_check and datetime.date.fromisoformat(o["date"])<SPLIT_DATE]
        obs_m2=[o for o in all_obs if o["sym"]==sym_check and datetime.date.fromisoformat(o["date"])>=SPLIT_DATE]
        am1=round(sum(o["alpha"] for o in obs_m1)/len(obs_m1),2) if obs_m1 else None
        am2=round(sum(o["alpha"] for o in obs_m2)/len(obs_m2),2) if obs_m2 else None
        print(f"\n  {sym_check}:")
        print(f"    Mitad 1: n={len(obs_m1):3d} alpha medio={am1}")
        print(f"    Mitad 2: n={len(obs_m2):3d} alpha medio={am2}")
        if am1 is not None and am2 is not None:
            if am1<0 and am2<0:
                print(f"    -> CONSISTENTE: falla en ambas mitades. Razon estructural probable.")
            elif am1<0 or am2<0:
                print(f"    -> INCONSISTENTE: solo falla en una mitad. Posible ruido/evento puntual.")
            else:
                print(f"    -> No falla en ninguna mitad de forma clara.")

    out={"mitad1":r1,"mitad2":r2,"split_date":SPLIT_DATE.isoformat(),
         "n_total_señales":len(snaps),"snapshots":snaps}
    ...
    print(f"{'='*70}")
    if r1 and r2:
        print(f"  Mitad 1 (2017-2021): alpha={r1['alpha_medio']:+.2f}% p_NW={r1['pvalue_nw']}")
        print(f"  Mitad 2 (2022-2026): alpha={r2['alpha_medio']:+.2f}% p_NW={r2['pvalue_nw']}")
    print(f"{'='*70}")
